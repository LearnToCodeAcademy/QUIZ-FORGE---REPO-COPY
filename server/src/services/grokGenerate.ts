...
import { geminiGenerateQuiz } from "../geminiClient";
...
const quizJsonSchema = {
  ...
  properties: {
    ...
    quiz_type: { type: "string", enum: ["mcq", "true_false", "fill_blank", "identification", "matching", "mixed"] },
        ...
        oneOf: [
          ...
          },
          {
            ...
            properties: {
              ...
              prompt: { type: "string" },
              choices: { type: "array", items: { type: "string" }, minItems: 2, maxItems: 2 },
              answer_index: { type: "number" },
              explanation: { type: "string" }
            }
          },
...
export async function generateQuizWithGemini(params: {
  ...
  try {
    ...
    const rawText = await geminiGenerateQuiz({
      systemPrompt,
      userPrompt: finalUserText
    });
    ...
    if (!validation.success) {
      return {
        ok: false,
        error: "Invalid JSON or schema mismatch from Gemini",
        raw: rawText,
        details: JSON.stringify(validation.error.issues, null, 2),
        debug: {
          fileIds: [],
